...
public class MjpegView extends SurfaceView implements SurfaceHolder.Callback{
    ...
    public void Start(String url, String user, String password, Handler parent_handler){
        handler = parent_handler;
        Stop();
        String[] params = {url, user, password};
        if(!suspending){
            SetThread();
            StreamReader = new DoRead();
            StreamReader.execute(params);
        }
    }
    ...
    public Bitmap getImage(){
        Bitmap imgBack;
        try{
            //imgBack = mIn.readMjpegFrame();
            imgBack = thread.getImgBack();
        }catch(Exception e){
            imgBack = null;
        }
        return imgBack;
